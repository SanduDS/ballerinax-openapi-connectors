...
public isolated client class Client {
    ...
    # This is a generated connector from [Bitbucket](https://developer.atlassian.com/bitbucket/api/2/reference/) OpenAPI 
    # Specification. The connector initialization requires setting the API credentials. Create an 
    # [Bitbucket account](https://bitbucket.org/product/) and obtain tokens following  
    ...
    # admin- returns repositories to which the user has explicit administrator access. owner-returns all repositories owned by the current user
    ...
    remote isolated function listWorkspaces(string workspace, ListWorkspacesRole? role = (), string? q = (), string? sort = ()) returns PaginatedRepositories|error {
    ...
    remote isolated function listPullrequests(string repoSlug, string workspace, ListPullrequetsState? state = ()) 
                                              returns PaginatedPullrequests|error {
    ...
    remote isolated function createNewIssueComment(string issueId, string repoSlug, string workspace, IssueComment payload) 
    ...
    remote isolated function getCommentByID(int commentId, string issueId, string repoSlug, string workspace) returns 
                                            IssueComment|error {
